...
set "NINJA_EXECUTE=%MULTIVERSE_DIR%\external\msys2\mingw64\bin\ninja.exe"

if exist "%CMAKE_EXECUTE%" if exist "%NINJA_EXECUTE%" (
    workon multiverse
    powershell -NoProfile -Command "%SET_NEW_PATH%; %CMAKE_EXECUTE% -S '%MULTIVERSE_DIR%' -B '%MULTIVERSE_DIR%\build' -G Ninja -DCMAKE_MAKE_PROGRAM=%NINJA_EXECUTE% -D CMAKE_C_COMPILER=%MULTIVERSE_DIR%\external\msys2\mingw64\bin\gcc.exe -D CMAKE_CXX_COMPILER='%MULTIVERSE_DIR%\external\msys2\mingw64\bin\g++.exe' -DCMAKE_INSTALL_PREFIX:PATH='%MULTIVERSE_DIR%' -Wno-deprecated -Wno-dev -DBUILD_MODULES=ON"
