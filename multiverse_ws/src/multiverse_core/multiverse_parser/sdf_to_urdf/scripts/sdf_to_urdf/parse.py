...
class Model(SpatialEntity):
    ...
    def plot(self, graph, prefix=""):
        ...
        for link in self.links:
            graph.add_node(
                full_prefix + link.name,
                label=full_prefix
                + link.name
                + "\\nrel: "
                + homogeneous2tq_string_rounded(link.pose)
                + "\\nabs: "
                + homogeneous2tq_string_rounded(link.pose_world),
            )
        subgraph = graph.add_subgraph(
            [full_prefix + link.name for link in self.links],
            "cluster_" + self.name,
            color="gray",
        ...
        for joint in self.joints:
            graph.add_edge(
                full_prefix + joint.parent,
                full_prefix + joint.child,
                label=full_prefix
                + joint.name
                + "\\nrel: "
                + homogeneous2tq_string_rounded(joint.pose)
                + "\\nabs: "
                + homogeneous2tq_string_rounded(joint.pose_world),
            )
...
class Axis(object):
    ...
    def __repr__(self):
        return "Axis(xyz=%s,%s lower_limit=%s, upper_limit=%s, effort=%s, velocity=%s)" % (
            self.xyz,
            " use_parent_model_frame=%s," % self.use_parent_model_frame if self.version >= 1.5 else "",
            self.lower_limit,
            self.upper_limit,
            self.effort_limit,
            self.velocity_limit,
    ...
    def add_urdf_elements(self, node, modelCBTjoint):
        ...
        limitnode = ET.SubElement(
            node,
            "limit",
            {"lower": str(self.lower_limit), "upper": str(self.upper_limit), "effort": str(self.effort_limit), "velocity": str(self.velocity_limit)},
        )
...
class LinkPart(SpatialEntity):
    ...
    def __repr__(self):
        return "%s geometry_type: %s, geometry_data: %s" % (
            super(LinkPart, self).__repr__().replace("\n", ", ").strip(),
            self.geometry_type,
            self.geometry_data,
        )
