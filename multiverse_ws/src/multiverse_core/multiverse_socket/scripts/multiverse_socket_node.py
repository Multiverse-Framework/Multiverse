    ...
    def create_publisher(self):
        ...
        for subclass in MultiverseRosPublisher.__subclasses__():
            if subclass.__name__ == publisher_name:
                return subclass(topic_name=topic_name,
                                rate=rate,
    ...
    def create_service(self):
        ...
        for subclass in MultiverseRosService.__subclasses__():
            if subclass.__name__ == service_name:
                return subclass(client_host=self.client_host,
                                client_port=self.client_port,
                                simulation_metadata=self.simulation_metadata,
                                **self.ros_base_prop)
...
class MultiverseRosSocket:
    ...
    def start(self):
        ...
        try:
            ...
            rospy.signal_shutdown(f"[{self.__class__.__name__}] Caught SIGINT (Ctrl+C), exiting...")
        finally:
            for subscriber in subscriber_list:
...
def main():
    ...
    multiverse_server = json.loads(args.multiverse_server.replace("'", '"')) if isinstance(args.multiverse_server,
                                                                                           str) else {
        "host": "tcp://127.0.0.1", "port": 7000}
