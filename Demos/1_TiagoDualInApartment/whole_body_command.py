...
class UpperBodyCommand(MultiverseClient, Node):    
    mujoco_upper_body_joint_state: Dict[str, float] = {
        "mujoco_head_1_joint": numpy.nan,
        "mujoco_head_2_joint": numpy.nan,
        'mujoco_torso_lift_joint': numpy.nan,
    ...
    joint_state_map: Dict[str, str] = {
        ...
        "torso_lift_joint": "mujoco_torso_lift_joint",
    ...
    def __init__(self, port: str, multiverse_meta_data: MultiverseMetaData, use_upper_body_publisher: bool = False, rate: float = 100.0) -> None:
        ...
        if self.use_upper_body_publisher:
            ...
            self.upper_body_publisher = self.create_publisher(Float64MultiArray, '/upper_body_position_controller/commands', 10)
    ...
    def joint_state_callback(self, msg: JointState) -> None:
        for name, position in zip(msg.name, msg.position):
            ...
            for group in (
                self.mujoco_upper_body_joint_state,
    ...
    def start(self):
        ...
        self.request_meta_data["receive"]["mujoco_torso_lift_joint"] = ["cmd_joint_linear_position"]
        for group in (
            self.mujoco_left_hand_joint_state,
            self.mujoco_right_hand_joint_state,
        ):
        ...
        arm_indices = [joint_indices[name] for name in self.mujoco_upper_body_joint_state if name in self.response_meta_data["receive"]]
        ...
        while True:
            ...
            response_meta_data = self.response_meta_data
        for object_name, object_attributes in response_meta_data["receive"].items():
            if object_name in self.mujoco_upper_body_joint_state:
        ...
        while any(value is None or numpy.isnan(value) for value in self.mujoco_upper_body_joint_state.values()) or \
              any(value is None or numpy.isnan(value) for value in self.mujoco_left_hand_joint_state.values()) or \
              any(value is None or numpy.isnan(value) for value in self.mujoco_right_hand_joint_state.values()):
            self.loginfo("Waiting for arm, left_hand, right_hand, and neck joint states to be read...")
            self.get_logger().info(f"Arm joint state: {self.mujoco_upper_body_joint_state}")
        ...
        start_arm_joint_state_array = [self.mujoco_upper_body_joint_state[joint_name] for joint_name in self.mujoco_upper_body_joint_state.keys()]
        ...
        for i in range(num_points):
            ...
            while rclpy.ok():
                ...
                if self.use_upper_body_publisher:
                    hand_state = start_arm_joint_state_array + interpolated_state
        ...
        self.loginfo(f"Moving arm from\n{self.mujoco_upper_body_joint_state} to\n{arm_start_joint_state}...")
        start_left_hand_joint_state_array = [self.mujoco_left_hand_joint_state[joint_name] for joint_name in self.mujoco_left_hand_joint_state.keys()]
        start_right_hand_joint_state_array = [self.mujoco_right_hand_joint_state[joint_name] for joint_name in self.mujoco_right_hand_joint_state.keys()]
        start_arm_joint_state_array = [self.mujoco_upper_body_joint_state[joint_name] for joint_name in self.mujoco_upper_body_joint_state.keys()]
        end_arm_joint_state_array = [arm_start_joint_state[joint_name] for joint_name in self.mujoco_upper_body_joint_state.keys()]
        for i in range(num_points):
            interpolated_state = [
                start + (end - start) * i / (num_points - 1)
            ...
            while rclpy.ok():
                start_time = time.time()
                if self.use_upper_body_publisher:
                    arm_state = interpolated_state + start_left_hand_joint_state_array + start_right_hand_joint_state_array
                    self.send_upper_body_command(arm_state)
                else:
                    self.send_mujoco_arm_joint_command(interpolated_state)
                rclpy.spin_once(self, timeout_sec=1.0/self.rate)
                stop_loop = True
                for joint_idx, joint_name in enumerate(self.mujoco_upper_body_joint_state.keys()):
                    current_value = self.mujoco_upper_body_joint_state[joint_name]
                    cmd_value = interpolated_state[joint_idx]
        ...
        self.loginfo(f"Current joint states: {self.mujoco_upper_body_joint_state}, {self.mujoco_left_hand_joint_state}, {self.mujoco_right_hand_joint_state}")
        self.loginfo(f"End arm joint states: {arm_start_joint_state}, End left_hand joint states: {left_hand_start_joint_state}, End right_hand joint states: {right_hand_start_joint_state}")
        self.loginfo("Starting the main loop to send and receive data.")
        self.loginfo(f"Arm indices: {arm_indices}, Left_hand indices: {left_hand_indices}, Right_hand indices: {right_hand_indices}")

        self.send_data = [self.sim_time]
        self.send_and_receive_data()

        try:
            self.create_timer(1.0/self.rate, lambda: self.loop(arm_indices, left_hand_indices, right_hand_indices))
            rclpy.spin(self)
        except KeyboardInterrupt:
            self.logwarn("KeyboardInterrupt: Exiting the program.")

    def loop(self, arm_indices, left_hand_indices, right_hand_indices) -> None:
        self.send_data = [self.sim_time]
        self.send_and_receive_data()
        receive_data = numpy.array(self.receive_data[1:])
        mujoco_upper_body_joint_state = receive_data[arm_indices].tolist()
        ...
        if self.use_upper_body_publisher:
            self.send_upper_body_command(mujoco_upper_body_joint_state + mujoco_left_hand_joint_state + mujoco_right_hand_joint_state)
        else:
            self.send_mujoco_arm_joint_command(mujoco_upper_body_joint_state)
