...
class MultiverseSmoother(MultiverseClient):
    def __init__(self, server_port: str, port: str, multiverse_meta_data: MultiverseMetaData) -> None:
        self._transport = "Zmq"
...
if __name__ == "__main__":
    ...
    multiverse_logger = MultiverseSmoother(server_port=args.server_port, port=args.port, multiverse_meta_data=multiverse_meta_data)
    ...
    if attribute_names == [""]:
        ...
        for object_name, object_attributes in multiverse_logger.response_meta_data["receive"].items():
            ...
            for attribute_name in object_attributes.keys():
                if attribute_name in ["joint_angular_position", "joint_linear_position", "joint_angular_velocity", "joint_linear_velocity"]:
        ...
        multiverse_logger.request_meta_data["receive"] = {}
        for object_name in object_names:
            if any(attribute_name not in ["joint_angular_position", "joint_linear_position", "joint_angular_velocity", "joint_linear_velocity"] for attribute_name in attribute_names):
