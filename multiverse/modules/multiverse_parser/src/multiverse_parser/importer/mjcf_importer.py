...
class MjcfImporter(Importer):
    ...
    def import_model(self, save_file_path: Optional[str] = None) -> str:
        ...
        for body_id in range(1, self._mj_model.nbody):
            self.import_body(body_id=body_id)
    ...
    def import_body(self, body_id):
        mj_body = self._mj_model.body(body_id)
        body_name = mj_body.name if mj_body.name is not None else "Body_" + str(body_id)
        ...
        if body_id == 1:
            ...
            parent_body_name = parent_mj_body.name if parent_mj_body.name is not None \
