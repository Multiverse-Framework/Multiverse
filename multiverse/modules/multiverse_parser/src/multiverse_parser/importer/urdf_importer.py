...
class UrdfImporter(Importer):
    ...
    def __init__(
        ...
        self._model = urdf.URDF.from_xml_string(urdf_string)
    ...
    def import_model(self) -> str:
        ...
        if self.config.model_name != self._model.get_root():
            self._world_builder.add_body(body_name=self._model.get_root(),
                                         parent_body_name=self.config.model_name)

        return self._world_builder.export()
