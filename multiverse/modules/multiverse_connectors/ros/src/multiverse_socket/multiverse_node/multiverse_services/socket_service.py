...
if USING_ROS1:
    ...
    from multiverse_interfaces.srv.Socket import Request as SocketRequest, Response as SocketResponse

from .multiverse_service import MultiverseService
...
class SocketService(MultiverseService):
    ...
    def _bind_request_meta_data(self, request: SocketRequest) -> SocketRequest:
        ...
        request_meta_data["meta_data"]["simulation_name"] = (
            "ros" if is_simulation_name_empty else self._simulation_name
        )
        request_meta_data["meta_data"]["length_unit"] = (
            "m" if meta_data.length_unit == "" else meta_data.length_unit
        )
        request_meta_data["meta_data"]["angle_unit"] = (
            "rad" if meta_data.angle_unit == "" else meta_data.angle_unit
        )
        request_meta_data["meta_data"]["mass_unit"] = (
            "kg" if meta_data.mass_unit == "" else meta_data.mass_unit
        )
        request_meta_data["meta_data"]["time_unit"] = (
            "s" if meta_data.time_unit == "" else meta_data.time_unit
        )
        request_meta_data["meta_data"]["handedness"] = (
            "rhs" if meta_data.handedness == "" else meta_data.handedness
        )
        ...
        for object_data in request.send:
            ...
            if is_object_name_empty:
                continue
            request_meta_data["send"][object_data.object_name] = [
                object_data.attribute_name
            ]
