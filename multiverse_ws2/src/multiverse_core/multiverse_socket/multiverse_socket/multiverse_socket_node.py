...
class MultiverseRosSocket(Node):
    def __init__(self) -> None:
        super().__init__('multiverse_socket')
    ...
    def start(self):
        ...
        multiverse_server = json.loads(f"{args.multiverse_server}".replace("'", '"'))
        ...
        #             self.get_logger().info(f"Start subscriber [{subscriber_name}] with {subscriber_prop}")
        ...
        if args.publishers is not None:
            ...
            for publisher_name, publisher_props in publishers.items():
                for publisher_prop in publisher_props:
                    ...
                    exec(f"from multiverse_socket.multiverse_ros_base.multiverse_publishers import {publisher_name}_publisher")
        ...
        #         self.get_logger().info(f"Start service server [{service_name}] with {service_prop}")
